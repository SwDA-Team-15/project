...
|----------|------------|-------------------------|
| **Single Responsibility (SRP)** | Partially satisfied | • `express.js`: mainly responsible for creating the app instance and exposing the public API; its job is essentially “factory + façade”, so the responsibility is quite clear.  <br>• `view.js`: focused on template resolution and delegation to the chosen engine; it does not deal with routing or HTTP details, which makes it a good SRP example.  <br>• `application.js`: aggregates configuration, middleware registration, routing delegation, template engine registration, rendering pipeline and server start‑up (`app.listen()`); this concentration of concerns breaks SRP.  <br>• `response.js`: handles status codes, headers, cookies, content negotiation, JSON helpers, file streaming, redirects and view rendering in a single large module; this breadth of behaviour suggests it should be decomposed into smaller pieces.  <br>• `utils.js`: provides several helper strategies (ETag, query parser, trust proxy, charset, content helpers); these are all “utility‑like”, but they cover different domains, so SRP is only partially respected. |
| **Open/Closed (OCP)** | Strong at extension points, weaker in core modules | • Middleware stack (`app.use()`): new cross‑cutting behaviour (logging, authentication, rate limiting, etc.) is usually added by registering middleware, without changing existing core code.  <br>• `view.js` + `app.engine()`: template engines are plugged in via a stable callback contract; new engines can be introduced without modifying `view.js` itself.  <br>• `application.js`: when the framework gains new lifecycle features (e.g. additional routing options or configuration flags), changes often end up here, so this module is not fully closed for modification.  <br>• `response.js`: new response helpers or HTTP features typically require editing this file, so extension is not purely additive at the component level. |
| **Liskov Substitution (LSP)** | Largely respected by design | • `request.js`: builds on `http.IncomingMessage`, keeping the original behaviour and adding convenience methods (params, query, accepts, etc.); it can still be used wherever a plain Node request is expected.  <br>• `response.js`: extends `http.ServerResponse` while preserving the basic semantics of writing headers and body; Express responses work in place of native responses for most Node‑style middleware.  <br>• Template engines (`pug`, `ejs`, etc.): each engine can replace another as long as it implements the expected `render(path, options, callback)` contract; call sites do not need to change.  <br>• Risk: because these contracts are informal, a badly behaved third‑party engine or helper can break substitutability at runtime even though the core tries to respect LSP. |
| **Interface Segregation (ISP)** | Mixed outcome | • Public app / middleware API: each middleware typically uses only a few methods on the app object, `req` and `res`, so in practice most user code depends on relatively small slices of the overall interface.  <br>• `application.js`: exposes many methods for configuration, routing and engine registration; callers conceptually depend on this wide interface even if they only use a subset.  <br>• `request.js`: adds a long list of helpers on top of the Node request; individual middlewares call only a small fraction of them, so the interface is not really segregated.  <br>• `response.js`: offers a broad API (status, headers, JSON, files, redirects, cookies, rendering); again, each middleware usually touches only a couple of these, which suggests that smaller, more specialised interfaces could improve ISP. |
| **Dependency Inversion (DIP)** | Followed at public boundaries, limited internally | • Middleware contracts: Express depends on the abstraction “function `(req, res, next)`” rather than any concrete implementation; users inject behaviour via these callbacks.  <br>• Strategy hooks in `utils.js`: ETag functions, query parsers and trust‑proxy strategies are provided by the user through configuration, so the core knows only about the expected behaviour, not about specific implementations.  <br>• Template engines: `view.js` calls engines through a simple render contract and does not depend on any particular templating library.  <br>• `request.js` and `response.js`: import concrete libraries such as `send`, `cookie`, `accepts`, `type-is` and similar directly; these modules do not sit behind abstractions, so they are tightly coupled to those packages and do not fully apply DIP inside the HTTP layer. |
